...
struct accumulate_type<binary_operation,current> {
    using type = current;
};

template<typename binary_operation, typename current, typename next>
struct accumulate_type<binary_operation,current,next> {
    using type = typename binary_operation::template type<current,next>;
};

template<typename binary_operation, typename current1, typename current2, typename current3>
struct accumulate_type<binary_operation,current1,current2,current3> {
    using type = 
    typename binary_operation::template type<current1, 
        typename binary_operation::template type<current2, current3> >;
};

template<typename binary_operation, typename current1, typename current2, typename current3, typename current4>
struct accumulate_type<binary_operation,current1,current2,current3,current4> {
    using type = 
    typename binary_operation::template type<current1, 
        typename binary_operation::template type<current2,
            typename binary_operation::template type<current3, current4>>>;
};

template<typename binary_operation, typename current1, typename current2, typename current3, typename current4, typename... rest>
struct accumulate_type<binary_operation,current1,current2,current3,current4,rest...> {
    using type = typename binary_operation::template type<typename accumulate_type<binary_operation,current1,current2,current3,current4>::type,typename accumulate_type<binary_operation,rest...>::type>;
};

template<typename binary_operation, typename current1, typename current2, typename current3, typename current4,
 typename current5, typename current6, typename current7, typename current8>
struct accumulate_type<binary_operation,current1,current2,current3,current4,current5,current6,current7,current8> {
    using type = 
    typename binary_operation::template type<current1, 
        typename binary_operation::template type<current2,
            typename binary_operation::template type<current3, 
                typename binary_operation::template type<current4,
                    typename binary_operation::template type<current5, 
                        typename binary_operation::template type<current6,
                            typename binary_operation::template type<current7, current8>>>>>>>;
};

template<typename binary_operation, typename current1, typename current2, typename current3, typename current4,
 typename current5, typename current6, typename current7, typename current8, typename... rest>
struct accumulate_type<binary_operation,current1,current2,current3,current4,current5,current6,current7,current8,rest...> {
    using type = typename binary_operation::template type<
        typename accumulate_type<binary_operation,current1,current2,current3,current4,current5,current6,current7,current8>::type,
        typename accumulate_type<binary_operation,rest...>::type
        >;
};
...
struct accumulate_type<binary_operation,
        ...
        typename binary_operation::template type<current2,
            typename binary_operation::template type<current3, 
                typename binary_operation::template type<current4,
                    typename binary_operation::template type<current5, 
                        typename binary_operation::template type<current6,
                            typename binary_operation::template type<current7, 
                                typename binary_operation::template type<current8, 
                                    typename binary_operation::template type<current9,
                                        typename binary_operation::template type<current10, 
                                            typename binary_operation::template type<current11,
                                                typename binary_operation::template type<current12, 
                                                    typename binary_operation::template type<current13,
                                                        typename binary_operation::template type<current14,
                                                            typename binary_operation::template type<current15,current16 >>>>>>>>>>>>>>>;
};
...
struct accumulate_type<binary_operation,
  ...
  current9,current10,current11,current12,current13,current14,current15,current16,rest...> {
    using type = typename binary_operation::template type<
        typename accumulate_type<binary_operation,current1,current2,current3,current4,current5,current6,current7,current8,
                                    current9,current10,current11,current12,current13,current14,current15,current16>::type,
        typename accumulate_type<binary_operation,rest...>::type
        >;
};
*/
